...
При этом сами переданные переменные неизменны поскольку изменяются только копии.
Для того что бы функция могла изменять значение переданных переменных их нужно передать по указателю либо по ссылке
------------------------------------------------------
Передача параметров по указателю 
для того что бы передать параметры в функицию должны параметры принимаемы функции должы быть указателями
Эти указатели нужно постоянно разыменовывать в теле функции 
как раз при разыминовании когда мы обращаемся к значению 
по адресу мы можем изменять значение переменно обьявленной в другой функции
Кроме того при вызове такой функции необходимо взять адресы передаваемых переменных
И тогда при вызове функции в нее копируется не значение а адреса передаваемых переменных
И разыминовывая эти адреса мы можем изменять оригинальные значения переменных
При вызове функции принимаемы параметры по указателю в нее копируются не значения а адреса передаваемых переменных
-------------------------------------------------------------
Передача параметров по ссылке 
Ссылка (reference) - это переменная которая содержит адрес другой переменной
В отличии от указателя ссылку не нужно разыминовывать компилятор сам это делает
Так же не нужно брать адрес переменной на которую ссылается ссылка компилятор и это сам делает
Но компилятор скрывает от програмиста адрес хранящийся в ссылке мы не можем его узнать и темболее не можем его изменить
Именно поэтому ссылку можно проинициализировать только при обьявлении
Ссылка это упрощенный вариант указателя 
Ссылки появились в языке с++ тоесть в языке с++ ссылок нееболы были только указатели
Ссылки используются только для передачи параметров функцию и больше ни для чего
Для того что бы передать параметры в функцию по ссылке принимаемы параметры в функции должны быть ссылками
В остальном передача параметров по ссылке синтаксически ничем не отличается в передаче параметров по значению
Но физически в функцию копируются адреса передаваемых переменных и поэтому функция может изменять 
-----------------------------------------------------------------------
Динамические массивы
Динамическим называется массив размер которого может быть задан на этапе выполнения программы 
Предварительно его можно вычеслить или ввести с калвиатуры 
Для того чтобы обьявить массив нужно обьявить указатель и выделить память спомощью оператора NEW следующим образом
int* arr = new int[n];
Оператор new выделяет непрерывный блок памяти для n элементов типа int и возвращает адрес выделенной памяти этот адрес мы записывааем в указатель
Динамические массивы передаются в функцию как и статические 
Когда динамический массив больше не нужен его обязательно нужно удалить из памяти 
С помощью delete
оператор delete нужно вызвать ровно столько раз сколько оператор new
*/